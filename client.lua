...
local robbing = false
local canRob = nil
local canRobBank = nil
local canRobAmmu = nil
...
AddEventHandler('BadgerBankRobbery:IsActive:Return', function(bool)
	...
	ESX.TriggerServerCallback('BadgerBankRobbery:canRobShop', function(cb)
	canRob = cb
	end, i)
	while canRob == nil do
		Wait(0)
	...
	ESX.TriggerServerCallback('BadgerBankRobbery:canRobAmmu', function(cb)
	canRobAmmu = cb
	end, i)
	while canRob == nil do
		Wait(0)
	...
	ESX.TriggerServerCallback('BadgerBankRobbery:canRobBank', function(cb)
	canRobBank = cb
	end, i)
	while canRob == nil do
		Wait(0)
		...
		if not robberyActive then
			...
			if canRobBank == true then
						...
						if IsControlJustReleased(0, 38) then -- E key
							...
							TriggerEvent("mythic_progressbar:client:progress", {
								...
								if not status and not IsEntityDead(GetPlayerPed(-1)) then
									...
									TriggerServerEvent('BadgerBankRobbery:finishbank')
								else
				...
				for _, bankcoords in pairs(config.bankcoords) do
					if (config.enableBanks == true) then
						if GetDistanceBetweenCoords(coords.x, coords.y, coords.z, bankcoords.x, bankcoords.y, bankcoords.z, true) < 5.0 then
							DisplayNotification('~r~Er is niet genoeg politie in dienst!')
						end
					end
				end
			end
			...
			if canRobAmmu == true then
						...
						if IsControlJustReleased(0, 38) then -- E
							...
							TriggerEvent("mythic_progressbar:client:progress", {
								...
								if not status and not IsEntityDead(GetPlayerPed(-1)) then
									...
									TriggerServerEvent('PrintBR:PrintMessage', config.robberySuccess)
									TriggerServerEvent('BadgerBankRobbery:finishammu')
								else
				...
				for _, ammunationcoords in pairs(config.ammunationcoords) do
					if (config.enableAmmunations == true) then
						if GetDistanceBetweenCoords(coords.x, coords.y, coords.z, ammunationcoords.x, ammunationcoords.y, ammunationcoords.z) < 5.0 then
							DisplayNotification('~r~Er is niet genoeg politie online')
						end
					end
				end
			end
			...
			if canRob == true then
						...
						if IsControlJustReleased(0, 38) then -- E
							...
							TriggerEvent("mythic_progressbar:client:progress", {
								...
								if not status and not IsEntityDead(GetPlayerPed(-1)) then
									...
									TriggerServerEvent('PrintBR:PrintMessage', config.robberySuccess)
									TriggerServerEvent('BadgerBankRobbery:finishstore')
								else
				...
				for _, shopcoords in pairs(config.shopcoords) do
					if (config.enable247 == true) then
						if GetDistanceBetweenCoords(coords.x, coords.y, coords.z, shopcoords.x, shopcoords.y, shopcoords.z) < 5.0 then
							DisplayNotification('~g~Er is niet genoeg politie in dienst!')
						end
					end
				end
			end
